...
# Load audio summaries from text files
def load_audio_summaries(directory):
    """
    Load audio summaries from text files in a directory.
    
    Args:
        directory (str): Path to the directory containing audio transcript files.
    ...
    for filename in sorted(os.listdir(directory)):
        if filename.endswith(".txt"):
            chunk_id = int(filename.split("_")[-1].split(".")[0])  # Extract chunk number
            with open(os.path.join(directory, filename), "r") as f:
                transcriptions[chunk_id] = f.read()
...
def assign_image_timestamps(image_summaries, frame_rate=30):
    ...
    for frame_name, summary in image_summaries.items():
        ...
        frame_number = int(frame_name.split("_")[1].split(".")[0]) * 100
...
def generate_final_summary(aligned_summaries):
    ...
    for time_segment, summaries in aligned_summaries.items():
        ...
        # Combine audio and image summaries into a single text
        combined_text = f"Audio Summary: {audio_summary}\nImage Summaries: {', '.join(image_summaries)}"
        
        # Use a language model (e.g., LLaMA, GPT) to generate a coherent summary
        # For simplicity, we'll just concatenate the summaries here
        final_summary[time_segment] = combined_text
